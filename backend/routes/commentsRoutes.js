...
commentsRouter.post("/accept/:commentID", (req, res) => {
  ...
  let deleteCommentQuery = `UPDATE Comments SET isAccept=true WHERE id = ${commentID}`;
...
commentsRouter.post("/reject/:commentID", (req, res) => {
  ...
  let deleteCommentQuery = `UPDATE Comments SET isAccept=false WHERE id = ${commentID}`;
  SabzLearnShopDB.query(deleteCommentQuery, (err, result) => {
    if (err) {
      res.send(null);
    } else {
